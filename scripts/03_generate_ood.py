...
def config():
    ...
    parser.add_argument("--ip_adapter_scale", type=float, default=1)
...
if __name__ == "__main__":
    ...
    device = args.device
    id_name_dict = load_id_name_dict(f"{args.dataset}.json")
    ...
    for synset in tqdm(sampled_keys,position=2):
        ...
        for i in tqdm(range(need_generate),position=3):
            try:
                ...
                images = og.generate_images_with_name(embeds, class_name,seed=seed,width=args.size,height=args.size
